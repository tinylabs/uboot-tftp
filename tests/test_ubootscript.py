...
def test_uboot_memset_uses_session_rambase_and_unsets_tmp():
    ...
    assert re.fullmatch(r"_[0-9]+", tmp_name)
...
def test_uboot_memcpy_uses_override_base_and_unsets_tmps():
    ...
    assert re.fullmatch(r"_[0-9]+", src_tmp)
    assert re.fullmatch(r"_[0-9]+", dst_tmp)
...
def test_uboot_crc32_gen_saves_and_restores_scratch_word():
    ...
    assert re.fullmatch(r"_[0-9]+", dest_tmp)
    assert re.fullmatch(r"_[0-9]+", saved_tmp)
